...
class ImageConverter(private val context: Context) {
    companion object {
        const val HARDCODE_SCALE = 1f
    }

    fun convert(img: Image, rotation: Int = 0): Bitmap {
        return nv21ToBitmap(yuv420toNV21(img), img.width, img.height, rotation)
    }
    ...
    fun scaleAndRotateBitmap(bitmap: Bitmap, scale: Float, rotate: Int): Bitmap =
        bitmap.scaleAndRotate(scale, rotate)
